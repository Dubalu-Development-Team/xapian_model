...
class TestManagerCreate:
    ...
    def test_with_explicit_id(self, mock_client):
        mock_client.put.return_value = {"id": "abc", "name": "Phone"}
        product = Product.objects.create(id="abc", category="electronics", name="Phone")

        mock_client.put.assert_called_once_with("/products/electronics", body={"name": "Phone"}, id="abc")
        assert product.id == "abc"

    def test_schema_auto_provision_on_412(self, mock_client):
        response_412 = MagicMock()
        response_412.status_code = 412
        exc = TransportError(response=response_412)
    ...
    def test_reraise_on_non_412(self, mock_client):
        response_500 = MagicMock()
        response_500.status_code = 500
        exc = TransportError(response=response_500)
        mock_client.put.side_effect = exc

        with pytest.raises(TransportError):
            Product.objects.create(category="electronics", name="Phone")

    def test_reraise_when_response_is_none(self, mock_client):
        exc = TransportError(response=None)
        mock_client.put.side_effect = exc
...
class TestManagerFilter:
    ...
    def test_returns_search_results(self, mock_client):
        ...
        result = Product.objects.filter(category="electronics", query="*", limit=10)
    ...
    def test_no_aggregations_attribute(self, mock_client):
        ...
        result = Product.objects.filter(category="electronics")
...
class TestManagerGet:
    ...
    def test_happy_path(self, mock_client):
        ...
        product = Product.objects.get(id="42", category="electronics")
    ...
    def test_volatile(self, mock_client):
        mock_client.get.return_value = {"id": "42", "name": "Widget"}
        Product.objects.get(id="42", category="electronics", volatile=True)
...
class TestDelete:
    ...
    def test_calls_client_delete(self, mock_client):
